...
| Dependency | `MySql.Data.dll` (not bundled — see Requirements) |
...
2. **MySQL / MariaDB** database + a user with privileges on it
3. **`MySql.Data.dll`** in the server's `Rocket/Libraries/` folder
...
- รถจะมีข้อมูลใน DB **เฉพาะตอนอยู่ในอู่** — `/gretrieve` จะลบ row ออก (รถกลับมาเป็นรถจริง)
